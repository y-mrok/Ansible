...
.. _ansible-command-kobun:

**************************************************
構文
**************************************************
:command:`ansible -h` の表示内容です。
...
.. _ansible-command-omona-parameter:
...
.. list-table::
   ...
   * - パラメーター
     - 意味
   * - pattern
     - | ・インベントリファイル内の管理対象ノードまたはグループを指定する
       | ・指定した管理対象ノードまたはグループに含まれるすべての管理対象ノード上でアドホックコマンドを実行する
   * - | -i
       | --inventory
       | --inventory-file
   ...
   * - | -m
       | --module-name
     - | ・アドホックコマンドとして実行する Ansible のモジュールを指定する
       | ・省略時は ``ansible.builtin.command`` モジュールが指定されたとことになる
   * - | -a
       | --args
     - アドホックコマンドとして実行する Ansible のモジュールのパラメーターに渡す値を指定する
   * - | -b
       | --become
     - 管理者モードでアドホックコマンドを実行する　＝　Linux コマンドの場合 :command:`sudo` コマンド付きで実行することと同じ意味
   * - | --ask-vault-password
       | --ask-vault-pass
